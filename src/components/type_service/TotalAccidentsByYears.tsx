...
export const TotalAccidentsByYears = () => {
    const {year} = useYear()
    const {totalYearAccidentesByServices} = useFetchTypeServices()
    const {data, isLoading} = useQuery({
        queryKey: ['totalYearsAccidentesByServices', year],
        queryFn: () => totalYearAccidentesByServices(year),
    })
        ...
        <div className="flex justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              Fallecidos por tipo servicio - a&ntilde;o {year}
            </h3>
