...
export const VehicleTypesMonths = () => {
  const { totalVehicleTypeByMonths } = useFetchVehicleTypes();
  const { year } = useYear();

  const { data = [], isLoading } = useQuery<TypeServiceData[]>({
    queryKey: ["typeVehiclesMonths", year],
    queryFn: () => totalVehicleTypeByMonths(year),
  });
  ...
  const uniqueServices = [
    ...new Set(data.map((item) => item.TIPO || "No especificado")),
  ];
  ...
  const series = uniqueServices.map((service) => ({
    ...
    data: uniqueMonths.map(
      (month) =>
        data.find(
          (d) => d.MONTH === month && (d.TIPO || "No especificado") === service
        )?.TOTAL || 0
    ),
  }));
